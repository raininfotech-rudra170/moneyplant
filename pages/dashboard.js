    ...
    <>
      <div className="cotainer-fluid pageHeading">
        <div className="container">
          <h1 className="text-light col-6">Dashboard</h1>
        </div>
      </div>
          ...
          <div className="c-dashboardInfo col-lg-3 col-md-6">
            <div className="wrap">
              ...
              <div className="d-flex justify-content-center align-items-center">
                ...
                <button className="ms-3 addBal" onClick={() => setIsOpen(true)}>+</button>
              </div>
            </div>
          </div>
          <div className="c-dashboardInfo col-lg-3 col-md-6">
          ...
          <div className="col-12 mb-4">
            <div className="border box-card">
              <Tabs defaultActiveKey="transaction" className="border-bottom justify-content-center justify-content-md-start">
                <Tab eventKey="transaction" title={<>Transactions <Badge bg="secondary" className='tabBadge'>{totalTxn}</Badge></>}>
                  <TxnHistoryTable user={user} sendDataToParent={sendDataToParent} />
                </Tab>
                <Tab eventKey="packages" title={<> My Packages <Badge bg="secondary" className='tabBadge'>{totalData}</Badge></>}>
                  <div className="d-flex align-items-center p-3 my-3 col-12 col-md-6 flex-wrap">
                    <div className="col-12 col-md-3 px-2 mb-3 mb-md-0">
                      ...
                      <select onChange={(e) => { setType(e.target.value); setPage(0) }}>
                        <option value=''>All</option>
                        {pkgtypes.map((d, i) => <option value={d._id} key={i}> {d.name} </option>)}
                      </select>
                    </div>
                    ...
                    <div className="col-12 col-md-3 align-self-end px-2 mb-3 mb-md-0">
                      <button className="btn btn-primary align-self-end col-12" onClick={getPkgHistry} disabled={pkgDataLoader}>Search</button>
                    </div>
                  </div>
                    ...
                    <table className="table text-center table-striped table-hover">
                      <thead>
                        <tr>
                          ...
                          <th className='sort-cell' onClick={() => sorting(1, order == -1 ? 1 : -1)}>Package <i className={`bi ${orderColumn === 1 ? order == -1 ? 'bi-caret-up-fill' : 'bi-caret-down-fill' : ''}`} /></th>
                          <th className='sort-cell' onClick={() => sorting(2, order == -1 ? 1 : -1)}>Amount <i className={`bi ${orderColumn === 2 ? order == -1 ? 'bi-caret-up-fill' : 'bi-caret-down-fill' : ''}`} /></th>
                          <th className='sort-cell' onClick={() => sorting(3, order == -1 ? 1 : -1)}>Purachased At <i className={`bi ${orderColumn === 3 ? order == -1 ? 'bi-caret-up-fill' : 'bi-caret-down-fill' : ''}`} /></th>
                        </tr>
                      </thead>
                      <tbody>
                        {pkgDataLoader ? <TableLoader colSpan={4} /> :
                          packageHisry.length ? packageHisry.map((d, i = 3) => {
                            return (
            ...
            <div className="box-card border p-3">
              {
                chartData.length ?
                  ...
                  <>
                    ...
                    <p className="text-center">Refer friends, when they buy one of our packages, EARN REWARDS and your earning's chart will appear here </p>
                  </>
              }
            </div>
          </div>
          {
            isOpen && <Modal show={isOpen} centered onHide={() => setIsOpen(false)}>
